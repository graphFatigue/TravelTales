...
export const useLocationInfo = (countryIds: number[] | number | undefined | null) => {
  // Normalize to always work with an array
  const normalizedCountryIds = Array.isArray(countryIds) 
    ? countryIds 
    : (countryIds !== undefined && countryIds !== null ? [countryIds] : []);
  ...
  const {
    ...
    queryFn: () => {
      ...
      return Promise.all(normalizedCountryIds.map(id => fetchCitiesByCountry(id)))
